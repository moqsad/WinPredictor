...
for value in range(1, 648):
    ...
    #change this indices if csv is updated( 0 based)
    innings_index = 6
    run_index = 15
    decision_index = 5
    toss_index = 16
...
df = pd.DataFrame(raw_data, columns=['label', 'Win', 'Loss', 'Tie'])
...
plt.ylim([0, max(df['Win'] + df['Loss'] + df['Tie'])])

# Adding the legend and showing the plot
plt.legend(['Win', 'Loss', 'Tie'], loc='upper left')
plt.grid()
plt.show()
