...
balling_strike_rate_mixed_index = 4
team_name_mixed_index = 10

win = 0
loss = 0
tie = 0
team1_balling_strike_rate = 0
team2_balling_strike_rate = 0

#648
for value in range(1,505):

    global team1_balling_strike_rate
    global team2_balling_strike_rate
    ...
    for line in reader2:
        ...
        if line[team_name_mixed_index] == team1_name:
            flag += 1
            print(line[balling_strike_rate_mixed_index] + '______________')
            team1_balling_strike_rate = float(line[balling_strike_rate_mixed_index])

        if line[team_name_mixed_index] == team2_name:
            flag += 1
            print(line[balling_strike_rate_mixed_index] + '______________')
            team2_balling_strike_rate = float(line[balling_strike_rate_mixed_index])

        if flag == 3:
            break

    print(team1_balling_strike_rate, team2_balling_strike_rate)
    print(winner)
    if(winner == 'tie'):
        tie = tie + 1
    elif winner == team1_name:
        if(team1_balling_strike_rate < team2_balling_strike_rate):
            win += 1
        else:
            loss += 1
    else:
...
title('Effect of Balling Strike Rate', bbox={'facecolor':'0.8', 'pad':5})
