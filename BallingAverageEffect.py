...
balling_average_mixed_index = 2
team_name_mixed_index = 10

win = 0
loss = 0
tie = 0
team1_balling_average = 0
team2_balling_average = 0

#648
for value in range(1,505):

    global team1_balling_average
    global team2_balling_average
    ...
    for line in reader2:
        ...
        if line[team_name_mixed_index] == team1_name:
            flag += 1
            print(line[balling_average_mixed_index] + '______________')
            team1_balling_average = float(line[balling_average_mixed_index])

        if line[team_name_mixed_index] == team2_name:
            flag += 1
            print(line[balling_average_mixed_index] + '______________')
            team2_balling_average = float(line[balling_average_mixed_index])

        if flag == 3:
            break

    print(team1_balling_average, team2_balling_average)
    print(winner)
    if(winner == 'tie'):
        tie = tie + 1
    elif winner == team1_name:
        if(team1_balling_average < team2_balling_average):
            win += 1
        else:
            loss += 1
    else:
...
title('Effect of Balling Average', bbox={'facecolor':'0.8', 'pad':5})
